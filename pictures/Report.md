...
### 关于几个Class的基本说明：
1. DataBase用于存放航班、Order的Arraylist，系统时间等基本信息，以及用户登录，管理员登录，用户注册，获取系统时间等基本方法
2. Admin： 存放管理员相关的变量和方法
3. Passenger： 存放顾客相关的变量和方法
4. Flight： 存放航班相关的变量和方法
5. Plane：用于存放飞机相关的变量和方法
> 本项目没有直接设定 Seat Capacity，而是初始化了两个飞机的机型，通过设定机型，来读取seat Capacity。后期调用方法 也可以直接修改。
...
##### 2. 对查询功能和高级查询功能的增强
  ...
  ![通过不完整信息对航班进行查询](superQueryFunc32.png)
##### 3. 对删除共功能的增强
...
调用了 Date Class的方法，创立 date 和 time的变量，得到格式化的，String类型的日期和时间，再赋值给DataBase中的当前时间（presentTime）和当前日期（presentDate）变量，再每次回到功能选择时，该方法会执行，来更新时间。
